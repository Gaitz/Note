...
### 第十四章 - 跨瀏覽器開發策略
  * 選擇要支援的哪些瀏覽器?、支援到什麼樣的程度?
  * 使用各家瀏覽器與各家作業系統行成的二維矩陣，並且填上分數決定支援程度。
  * 考慮, 
    * 目標使用者的期望與需求, 
    * 瀏覽器市占率, 
    * 為了支援特定瀏覽器，所需投入的資源。
...
#### 開發時的五大問題
  * 瀏覽器的 Bug
  * 瀏覽器的 Bug 修正, 
    * 使用瀏覽器標準檢驗 Bug, 小心針對在 bug 上的解決方案，可能在未來修正後導致的錯誤。
  * 外部程式碼, 
    * 封裝我們的程式碼
    * 處理貪婪ID, 避免在 `id`, `name` 上使用屬性標準名稱，可能會遇到瀏覽器的舊實作上產生的錯誤。
    * 正確的樣式表與程式載入順序
  * 瀏覽器的迴歸錯誤 (regression),
    * **密切注意即將發行的瀏覽器版本**, 盡早做出對策。
  * 瀏覽器所缺少的功能

#### 實作策略
  * 安全的跨瀏覽器修復, 目標是為所有瀏覽器提供一致的解決方案
    * 不會對其他瀏覽器產生負面影響或副作用
    * 不需要對瀏覽器做功能偵測
  * 功能偵測與填充工具 (polyfill)
    * 功能偵測後的策略 1. 提供縮減版體驗, 2. 不執行任何 JavaScript, 提供無 JavaScript 版 HTML, 3. redirect 到簡單版本
  * 無法程式測試的瀏覽器問題
    * 事件處理器綁定與否
    * 事件觸發與否
    * CSS屬性效果
