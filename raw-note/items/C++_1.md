...
- Virtual function (virtual member) 用來處理繼承關係中被重複定義的函式 (overriding)
  - 提供機會通過 parent class pointer 或 reference 來呼叫其他版本的 override function
- 使用 `virtual` keyword 在 parent class 中的 member function
- 屬於 runtime polymorphism
- Virtual function 的重點在於
  - 使用 pointer 或 reference 指向 base class 中的 virtual function
  - 這樣的 runtime function binding
- 一般的繼承關係中的 function overiding 是屬於 compile time function binding, 在 compile time 就決定好對應的 function
  - 無法通過 parent class pointer 或 reference 去呼叫 parent class 的 function
- Limitations of virtual functions
  - Slower, 因為屬於 runtime binding, 無法在 compile time 進行優化, 因此執行效能較差
  - Difficult to debug, 在多層的繼承關係中, 如果使用 virtual function 很難判斷是哪個 function 在執行, (因為更複雜的機制, 導致更難理解, 更難除錯)

C++ Virtual Destructor

- **Best Pratice**, 在使用 virtual function 時, 一同建立對應的 virtual destructor (就算沒有做任何事情)
- 讓 delete object 時的 destructor 能夠都被正確的呼叫
...
- 在 C++ 中的 abstract member function 即 pure virtual function
- 定義方式為 `virtual function_definition = 0;`
- 換句話說在 C++ 中的 abstract clsss 就是擁有一個以上的 pure virtual function 定義的 class
...
- 使用 pointer 的方式, 在 inheritance hierarchical tree 中移動
...
- Exception 是程式在 runtime 時出現的異常狀況
- Synchronous exception, 程式內部遇到的錯誤, 像是除 0 或型別轉換錯誤
- Asynchronous exception, 程式外部的錯誤, 超出程式本身能控制的, 像是硬碟錯誤
- `try`, 在 try block 中的程式碼可能會丟出 (`throw`) exception
- `catch`, 捕捉特定的 exception 來做特定的處理
  - 一次捕捉所有的例外, `catch (...) {}`
- `throw`, 主動丟出例外 (可以是 Class 或者原始型別)
  - 允許 re-throw exception, `throw;`
- Why do we need Exception Handling in C++
  - 分離錯誤處理程式碼與正常程式流程
    - 提高可維護性與閱讀性
  - 每個函式只處理特定的例外, 沒有被捕捉的例外會繼續向上傳遞, 因此可以由不同的函式處理不同的例外
  - 集合錯誤型別, Error Types 可以通過 namespace 或 classes 的方式分類使用
- 如果程式丟出例外, 而沒有被任何的 catch 捕捉到時程式會終止 (terminiate)
- Best pratice, 如果有使用到 `throw` 的函式, 應該在函式簽名上加註 `throw( types )`, 讓函式呼叫方知道應該處理例外
- 丟出例外並被捕捉時代表原先的 scope 已經結束跳轉到其他的函式內部, 因此原先 scope 中的 object 會進行 destruct
- Limitations of Exception Handling
  - 沒有正確處理的 exception handling 可能造成 resouce leaks

C++ Stack Unwinding

- Stack unwinding 是 function entries 從 function call stack 在 runtime 移除的過程
  - Local objects 會 destroy
- Exception handling 時, function call 會依序搜尋且跳轉到 exception handler
  - 在 exception handler 之前的 function call 會進入 stack unwinding 流程

C++ User-Defined Exceptions

- Exception handling with class
  - exception 使用使用者定義的 class
...
- File handling 使用於儲存永久資料在電腦上
- 使用 file handling 儲存資料在硬碟上
...
- Streams in C++
  ...
  - Stream 即資料流 (the flow of data in a sequence)
  - Input 與 Output 時產生的資料流
...
- Open file
  ...
  - By open method
- Modes
  - in, file open for reading, input
  - out, file open for writing, output
  - binary, 作用於 binary mode 取代 text
  - ate, 從檔案結尾開始寫入, at end
  - app, 所有的操作都在檔案結尾開始進行, append
  - trunc, 清除開啟檔案前的內容, truncate
- 預設模式
  - `ifstream`, `ios::in`
  - `ofstream`, `ios::out`
...
- 在 C 中, 可以使用 `freopen()` 進行 redirect 檔案的 pointer 到其他的 stream 上
- 在 C++ 中, stream 只是單純的 class
...
- Template, 允許傳入 data type 作為參數, 因此可以不需要針對不同的 data types 撰寫相同的邏輯而產生多個函式
- `template <typename T>`
- Function templates
- Class templates
- 為 template argument 提供預設值
  - 預設的型別 `=`
- Function overloading 與 template 機制都是 OOP 中 polymorphism 實現的方式
- Template argument deduction
  - 可以省略傳入 type 而是讓 compiler 自動推定
  - 在 C++98 以後支援 Function template argument deduction
  - 在 C++17 以後支援 Class template argument deduction

C++ Template Specialization

- template specialization
  - 在 template 中針對不同的 type 撰寫不同的邏輯
- 像是使用 function overloading 一樣, 只是作用在 template function
  - 宣告時使用 `template<>` + function overloading
- Class template 同樣適用

C++ `using` Keyword

- 使用 `using` 來指定 namespace
  - `using namespace`
- 使用 `using` 來指定 inheritance
- 使用 `using` 來宣告型別的別名 (aliasing)
  - 範例: `using ll = long long;`
- 使用 `using` 來明確指定 namespace function
  - 範例: `using std::cout;`
...
- C++ 內建的 template classes, 用來提供常見的 data structures, functions 與演算法
- STL 是 template library 因此是 generic
- 內建的演算法, 通常是最佳演算法, 在 runtime 有最好的效能
- 主要分成幾類
  - Containers
  - Algorithms
  - Iterators
  ...
  - Adapters
- Algorithms
  - Sorting, Searching, Important algorithms, Array algorithms, Partition operations, valarray class
- Containers
  - vector, list, deque, arrays, forward_list, queue, priority_queue, stack, set, multiset, map, multimap, unordered_set, unordered_multisest, unordered_map, unordered_multimap
- Advantages
  - Reusability, 重用性
  - Efficient algorithms, 高效能
  - Improved code readability, 可閱讀性
  - Large community of users, 廣泛被使用, 因此很多社群資源
- Disadvantages
  - Learning curve, 學習曲線高
  - Lack of control, 控制權有限
  - Performance, 在特定情況下可能效能更差 (像是數量級較小時)
...
- 以 class template 的方式實作
- Sequence containers
  - array, Static continuous array
...
- Unordered associative containers, 使用 hash 因此比 sorted 版本在 search 效能更佳 (best O(1), worst O(n))
  - unordered_set,
  - unordered_map,
  - unordered_multiset,
  - unordered_multimap,
- Container adapters, 提供介面轉換, 轉換 sequential containers
  - stack, 轉換介面成 stack
  - queue, 轉換介面成 queue
  - priority_queue, 轉換介面成 priority queue

STL Vector

- Vector 是 dynamic array 會自動的 resize array
- Insert 可能會觸發 resize 因此每次執行的效能不一定
- 沒有觸發 resize 的 insert 與 erasing 效能都是 O(1)
- `<vector>` header
- `std::vector<dataType> vectorName;`
- Initialization
  - Using list, 使用 `{ value1, value2, ... }`
...
- Initialization
  - With values
  - From another pair
  - `make_pair(value1, value2)` function
  - `{ , }`
- Functions
  - `swap()`, swap 兩個同型別 pairs 的值
  - `tie()`, 解構, 可以配合 `ignore` keyword 作為不需要的佔位
- Operators
  - `=`, assign
  - `==`, `!=`, equal compare
  - `>=`, `<=`,
...
- Ordered set 底層使用 Binary Search Tree 實作
- `<set>` header
...
- 屬於 container adaptor 成 LIFO 的結構介面
- 底層選用的 container 可以是 vector, deque, list
  - deque 是預設值
- `<stack>`

STL Queue

- 屬於 container adaptor 成 FIFO 的結構介面
- `<queue>`

STL Priority Queue

- 屬於 container adatpor 成 Heap 的資料結構介面
- Priority queue 的第一個元素, 會是最大值或最小值, 並且元素以 non-increasing 或 non-descreasing 的模式排序
  - 預設是最大值
- 內部使用 array 或 vector 實現
- `<queue>`

STL Deque

- Deque, double-ended queues, sequence container
  - 類似 vector 但是增加隊首跟隊尾的操作函式, 在開頭或結尾進行 insert 與 delete 效能更佳
  - Access element O(1)
  - 任意增加與刪除 O(N)
  - 開頭或結尾新增與刪除 O(1)
- `<deque>`
...
- List, doubly linked list, sequence container, 允許不連續的記憶體
- 相較於 vector 走訪的速度較慢 O(N), 但是在找到位置後的新增與刪除較快 O(1)
- `<list>`
...
- 與 List (doubly linked list) 比起來空間使用較少 (只存單向), 缺點是無法雙向走訪
- `<forward_list>`

STL Map

- Map, associative container, 每個元素都有 key 跟 value
- Sorted 版本, 每個 key 都是唯一的
- insert, count, erase, 效能都是 O(log(n))
- `<map>`

STL Multimap

- Multimap, 類似於 map 但是允許相同的 key 對應不同的 value 存在
- 並且 element 會以 key sorted 的方式儲存
- `<map>`

STL Bitset

- 空間最佳化 array of bools 的實作
- 每個位置都只有 1 bit 因此在空間利用上最佳化
- Size 是 static 的, 在 compile time 就固定
- `<bitset>`
- `bitset<size> variableName(initialization)`
- Initialization
  - 預設會初始化為 0
  - 傳入 decimal integer 會解構成 binary form 儲存
  - Binary string

STL Unordered Sets

- `unordered_set` 以 hash table 實作, 因此不保證順序,
  - 但是 operations 效能最佳化成 O(1), worst case 是 O(n)

STL Unordered Multiset

- hash table 版本的 multiset
- 效能最佳化 O(1) 但是不保證順序 (unsorted)
...
- Iterator 是一個像是 pointer 的 object 用來指向 container 內的元素
- 使用 iterator 在 container 中走訪
- Iterator 抽象化 container 的走訪, 是 algorithm 適用於各種不同 containers 的關鍵
- 依據資料結構的不同, 因此 iterator 所提供的 feature 也不同
...
- 五種類型 iterators
  - Input iterators, 最有限的功能, 單向走訪
  - Ouput iterators, 最有限的功能, 單向賦值
  - Forward iterators, 等同於 input iterator + output iterator, 因此也是單向
  - Bidirectional iterators, 等同於雙向的 forward iterator
  - Random-access iterators, 等同於 bidirectional iterator 加上能夠 random-access
- 五種 iterators 是包含關係
  - Random-access > Bidirectional > Forward iterator = Input iterator + Output iterator

C++ Input Iterators

- Input iterator, 屬於功能最少的一類 iterator
- 只能循序的進行單向走訪, 並且是 read-only (允許取值),
- Features
  - Usability, 單向 Read-only 的循序走訪
...
- Output iterator, 屬於功能最少的一類 iterator
- 類似 input iterator 只是差別在於不允許取值, 但是允許賦值 (assign)

C++ Forward Iterators

- Forward iterator 就是 input iterator + output iterator
  - 仍然是單向走訪, 但是允許取值跟賦值

C++ Bidirectional Iterators

- Bidirectional iterator 等同於允許雙向走訪的 forward iterator
  - 即存在 decrementable, `--`

C++ Random Access Iterators

- 最多功能的 iterator
  - 包含全部 input iterator, output iterator, forward iterator, bidirectional iterator 的功能
  - 加上允許 random access 任何位置的元素
- Usability
- Equality / Inequality comparison `==`, `!=`
- Dereferencing, `*`
- Incrementalbe, `++`
...
- Iterator 與 Pointer 非常類似
- 差別在於
- Pointer 是直接操作 memory 的概念, 並且允許刪除 (`delete`) pointer, 取值能自由的轉換型別
- Iterator 是 object 存在於 container class 中, 不允許刪除 (由 container class 所控制), 只能取得固定型別的資料
...
- Preprocessor 是 source code 再進入 compilation 階段之前運作的程序
- 順序是
  - `.c` source code ->
  - `.i` expanded source code (from preprocessor) ->
  - `.o` object code (compiled by compiler) ->
  - `.exe` executable file (by linker)
- Preprocessor directives
  - 都是以 `#` 開頭
  - 告知 compiler 需要先經過 preprocess 才進行 compiling
- List of preprocessor directives
  - `#define`, macro
  - `#undef`, macro
  - `#include`, include a file in the source code
  - `#ifdef`
  - `#if`
  - `#else`
  - `#endif`
- 主要分成四類
  ...
  - Other directives
- Macros
  - `#define token value`, 像是在 source code 中進行單純的字串取代 (replace)
  - With Arguments, 範例: `#define foo(a, b) a + b`
- File Inclusion
  - `#include`, 告知 compiler 需要引入其他檔案的內容進入 source code
  - Standard header files, 使用 `< >`
  - User-defined header files, 使用 `" "`
- Conditional Compilation
  - `#if`, `#ifdef`, `#ifndef`, `#else`, `#elif`, `#endif`
- Other Directives
  - `#undef`, undefine 已經存在的 marco
  - `#pragma`, 開關某些 compiler features, 屬於 compiler-specific
  - `#line`, Line control
  - `#error`, Error directive, 主動丟出 compiler error

Difference between C++ Preprocessor Directives and Function Templates

- 使用 Function Template 取代 Preprocessor Directives 的功能
  - 讓 compiler 提供 type checking
  - Pre-processor 更容易出錯
...
- Namespace 提供定義 (define) 與宣告 (declare) identifier 的空間
- Namespace 建立出 scope
- Advantage
  - 避免 name colliion
  - 限縮 global scope
...
- Using namespace
  ...
  - scope operator, `::`
- Nested namespace
  - 允許存在 nested namespaces
- Namepace Aliasing
  - `namespace NewName = CurrentName;`
- Namespace 是 C++ 的功能, C 無法使用

Extending C++ Namespace and Unnamed Namespace

- Extending namespace
  - 重複名稱的 namespace 多次定義會自動變成 extending
- Unnamed namespace
  - `namespace {}`
  - 定義時不提供名稱, 而是讓 compiler 自行生成
  - Unnamed namespace 定義出的內容只能被當前的檔案使用
  - 用來取代 static declaration of variables 的功能

C++ Inline Namespaces

- 使用於 nested namespace
- `inline namespace`
- 等同於在 parent namespace 呼叫使用 (`using`) child namespace
  - 讓 child namespace 的 identifier 可以直接通過 parent namespace 呼叫
...
- Multithreading 允許可以同時執行兩個以上的運行, 最大化利用多核心 CPU
- 每個程式運行部分稱為 thread, thread 是輕量化的 process
- C++11 之後才有標準支援 multithreading
  - 原先使用的 POSIX thead 或 `<pthreads>` 具有可移植性問題
- C++11 以後使用 `<thread>`
- `std::thread thread_object (callable);`
- 流程是
  - 建立 thread object
  - 傳遞 callable object 給 thread object
- Callable
  - Function pointer
  - Lambda expression
  - Function object
    - Class with function overload `()` operator
...
- Pointer 去存取 heap memory
- Normal pointer 的缺點
  - Memory leaks, 沒有正確釋放記憶體導致的 memory leak
  - Dangling pointers, 可能存在 pointer 可以指向已經被刪除的記憶體空間
  - Wild pointers, 宣告 pointer 但是沒有指定任何的初始值
  - Data inconsistency, 資料不一致
  - Buffer overflow, 寫入的資料大於預定的空間, 影響到其他的 memory, 造成安全漏洞
- Smart pointer
  - 很多其他的程式語言有 garbage collection 機制, 會自動清除沒被使用到的記憶體
  - 對於 C++ 來說的 garbage collection 機制就是 smart pointer
  - Smart pointer 是對於 normal pointer 的一個 wrapper class
    - 換句話說, 是利用 class destructor 會自動被呼叫的機制, 來自動的觸發記憶體回收
- Smart pointer 實作[參考](https://www.geeksforgeeks.org/smart-pointers-cpp/)
- C++ 內建提供的 smart poniter 實作, `<memory>`, 可以分成四種類型
  - `auto_ptr`, (deprecated after C++11)
  - `unique_ptr`, 每個實體只能由一個 pointer 指向
  - `shared_ptr`, 每個實體可以由多個 poniter 指向
  - `weak_ptr`, 輕量版的 shared_ptr, 不持有所有權 (不會增加 reference counter)

Differences between Different C++ Smart Pointers

- `unique_ptr` 是 C++11 以後支援用來替代 `auto_ptr` 更有安全性的版本
- `unique_ptr` 使用在保證單一 ownership
- `shared_ptr` 允許一個資源擁有多個 pointer 指向並且都擁有 ownership
  - 實現 reference counting ownership model
  - 只有在 reference count 歸零時才進行 garbage collection
  - 兩個 resource 互相持有對方的 pointer 時會導致 cyclic dependency
- `weak_ptr` 輕量版的 `shared_ptr` 並不會增加 ownership 的版本
  - 避開 `shared_ptr` 可能發生 cyclic dependency 導致 reference count 永遠不會歸零, 因此 memory leak 的問題

Type of ‘this’ Pointer in C++

- class 中的 `this` pointer 指向實體本身
...
- 把函式當作參數傳入另一個函式中
  - 範例像是 sort function 可以傳入 comparator function
- 實現方式可以分成三種
  - Passing as pointer
  - Using `std::function<>`
  - Using Lambdas
- Passing as pointer
  - 傳遞 Function pointer, 對於 function identifier 使用 `&` 取得 function pointer
  - 宣告時使用 pointer 宣告傳入的函式型別
- Using `std::function<>`
  - C++11 以後支援
  - 宣告時使用 `function<RetunrType(ParameterTypes)>` 宣告傳入的函式型別
- Using Lambdas
  - 傳入 inline 定義的匿名函式 (lambda)

C++ Signal Handling

- Signal 用來介入 OS 來影響正在進行的 task
- Signals
  - `SIGINT`
  - `SIGTERM`
  - `SIGBUS`
...
- 傳遞型別作為參數的函式實作
- Generic Programming 實現無關型別的實作方式
  - 減少重複的程式碼, 提高程式碼的重用性
  - 避免使用 function overloading
- C++ 中以 template 的方式實現 generic programming
- `template <>`, `typename`, `class`
- 實現 generic function 與 generic class
